...
def checkModel(x_rob,y_rob,theta,point):
    ...
    for cont in controls:
        ...
        prior = np.array(point)
        now = np.array((xNew,yNew))

        dist = np.linalg.norm(prior-now)
...
for _ in range(10000): 
    ...
    contrainedX, constrainedY,constrainedTheta = checkModel(x_rob,y_rob,theta,added_point)

    #print added_point = np.array((contrainedX))
